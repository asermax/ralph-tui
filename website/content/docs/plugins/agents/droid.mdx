...
| `skipPermissions` | boolean | `false` | Enables `--skip-permissions-unsafe` for autonomous runs |
...
Factory Droid normally asks for permission before executing tool actions. Set `skipPermissions = true` to bypass prompts:
...
Only enable skip permissions for trusted tasks. The agent will run without interactive approvals.
</Callout>
